<template>
  <header class="header-container" :class="{ 'search_box-enabled': searchBox }">
    ...
    <div v-if="hasMenu" class="menu-container me-2">
      ...
      <div class="item" v-if="userStore.user" @mouseover="openPopover" @mouseout="closePopover">
        <router-link to="/user/purchase" @mouseover="openPopover"
          >{{ userStore.user.lastName }} {{ userStore.user.firstName }}
        </router-link>
        <custom-popover :options="popoverOptions" :open="showPopover"></custom-popover>
      </div>
